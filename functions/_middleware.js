...
export async function onRequest(context) {
  ...
  // 1. Allow static assets and test client pages to pass through
  if (
  ...
  // 2. Validate client_request_token for all SSO entry points (/, /index.html, /accounts-login.html)
  const token = url.searchParams.get('client_request_token');

  if (!token || token.trim() === '') {
    // Return authentic HTTP 400 Bad Request with Google 400 Error Page
  ...
  // 3. Valid token present -> Serve the Google login authentication application with HTTP 200
  return context.next();
}
